...
class DailyWorkReportController extends GetxController with GetTickerProviderStateMixin {
  ...
  final checkWeather = ['맑음', '흐림', '비'].obs;
  ...
  Future getDocData() async {
    try {
      ...
      await docDataList.asyncForEach((item) async {
        if (item.documentColumnValue!.contains('file')) {
          // 이미지 저장
          imageUrl[item.documentColumnValue.toString()] = value(item.documentDataValue);
        } else {
          // text 저장
          textController[item.documentColumnValue.toString()] = TextEditingController(text: value(item.documentDataValue));
          ...
          // 날씨
          if (item.documentColumnValue == 'weather') {
            if (checkWeather.contains(value(item.documentDataValue))) {
              weatherValue.value = await value(item.documentDataValue);
            } else {
      ...
      if (e.toString().toLowerCase().contains('connection refused') || e.toString().toLowerCase().contains('connecting timed out')) {
        ...
        if(Loader.isShown) {Loader.hide();}
        Get.showSnackbar(Ui.ErrorSnackBar(message: '작업일지 조회 중 에러가 발생하였습니다.'));
        throw Exception(e);
      }
    }
  }
  ...
  Future saveDocData() async {
    try {
      ...
      if (e.toString().toLowerCase().contains('connection refused') || e.toString().toLowerCase().contains('connecting timed out')) {
        ...
        if(Loader.isShown) {Loader.hide();}
        Get.showSnackbar(Ui.ErrorSnackBar(message: '작업일지 저장(insert) 중 에러가 발생하였습니다.'));
        throw Exception(e);
      }
    }
  }

  /// daily work report - update
  Future updateDocData() async {
    try {
      ...
      if (e.toString().toLowerCase().contains('connection refused') || e.toString().toLowerCase().contains('connecting timed out')) {
        ...
        if(Loader.isShown) {Loader.hide();}
        Get.showSnackbar(Ui.ErrorSnackBar(message: '작업일지 저장(update) 중 에러가 발생하였습니다.'));
        throw Exception(e);
      }
    }
  }
