...
class WorkSafetyMeetingRiskAssessmentEducationJournal2Controller  extends GetxController with GetTickerProviderStateMixin {
  ...
  // 사진, 시간 맵
  Map photoTime = {
    'work_asng_oper_photo_file_name_1' : 'work_asng_oper_photo_1_time',
    'work_asng_oper_photo_file_name_2' : 'work_asng_oper_photo_2_time',
    'work_asng_oper_photo_file_name_3' : 'work_asng_oper_photo_3_time',
    'work_asng_oper_photo_file_name_4' : 'work_asng_oper_photo_4_time',
    'work_asng_oper_photo_file_name_5' : 'work_asng_oper_photo_5_time',
    'work_asng_oper_photo_file_name_6' : 'work_asng_oper_photo_6_time',
  };

  Map photoLatitude = {
    'work_asng_oper_photo_file_name_1' : 'work_asng_oper_photo_1_latitude',
    'work_asng_oper_photo_file_name_2' : 'work_asng_oper_photo_2_latitude',
    'work_asng_oper_photo_file_name_3' : 'work_asng_oper_photo_3_latitude',
    'work_asng_oper_photo_file_name_4' : 'work_asng_oper_photo_4_latitude',
    'work_asng_oper_photo_file_name_5' : 'work_asng_oper_photo_5_latitude',
    'work_asng_oper_photo_file_name_6' : 'work_asng_oper_photo_6_latitude',
  };

  Map photoLongitude = {
    'work_asng_oper_photo_file_name_1' : 'work_asng_oper_photo_1_longitude',
    'work_asng_oper_photo_file_name_2' : 'work_asng_oper_photo_2_longitude',
    'work_asng_oper_photo_file_name_3' : 'work_asng_oper_photo_3_longitude',
    'work_asng_oper_photo_file_name_4' : 'work_asng_oper_photo_4_longitude',
    'work_asng_oper_photo_file_name_5' : 'work_asng_oper_photo_5_longitude',
    'work_asng_oper_photo_file_name_6' : 'work_asng_oper_photo_6_longitude',
  };
  ...
  initState() async {
    logger.d('WorkSafetyMeetingRiskAssessmentEducationJournal2Controller - initState');
    try {
      docColList.clear();
      ...
      switch(crud.value) {
        ...
        case 'crud-c':
          ...
          break;
        default:
          logger.d("WorkSafetyMeetingRiskAssessmentEducationJournal2Controller initState - invalud crud.value");
      }
  ...
  Future getDocData() async {
    try {
      ...
      if (e.toString().toLowerCase().contains('connection refused') || e.toString().toLowerCase().contains('connecting timed out')) {
        ...
        if(Loader.isShown) {Loader.hide();}
        Get.showSnackbar(Ui.ErrorSnackBar(message: '작업전후 안전회의 및 위험성평가 후 교육일지 1 조회 중 에러가 발생하였습니다.'));
        throw Exception(e);
      }
    }
  }
  ...
  Future getCamera(String path) async {
    try {
      ...
      if (image != null) {
        ...
        textController[photoLatitude[path]]!.text = position.latitude.toString();
        textController[photoLongitude[path]]!.text = position.longitude.toString();
        ...
        textController[photoTime[path]]!.text = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
  ...
  /// 작업전후 안전회의 및 위험성평가 후 교육일지 1 - insert
  Future saveDocData() async {
    try {
      ...
      List<String> fileType = ["_File", "XFile"];

      /** image data */
      for (var item in imageUrl.keys) {
        data[item] = await (fileType.contains(imageUrl[item].runtimeType.toString()) ? dio.MultipartFile.fromFileSync(imageUrl[item].path) : imageUrl[item]);
      }
  ...
  /// 작업전후 안전회의 및 위험성평가 후 교육일지 1 - update
  Future updateDocData() async {
    try {
      ...
      if (e.toString().toLowerCase().contains('connection refused') || e.toString().toLowerCase().contains('connecting timed out')) {
        ...
        if(Loader.isShown) {Loader.hide();}
        Get.showSnackbar(Ui.ErrorSnackBar(message: '작업전후 안전회의 및 위험성평가 후 교육일지 1 저장(update) 중 에러가 발생하였습니다.'));
        throw Exception(e);
      }
    }
  }
